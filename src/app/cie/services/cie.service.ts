...
export class CieService {
  ...
  getRegistros(nombre_cuenta: string = '-', mes: number = 0, anio: number = 0, concepto: string = '-', empresa: string = '-', num_proyecto: number = 0, responsable: string = '-', clasificacionPY: string = '', offset: number, limit: number) {
    return this.http.get<CieRegistrosPaginadosResponse>(`${this.baseUrl}api/Cie/Registros/true/${nombre_cuenta}/${mes}/${anio}/${concepto}/${empresa}/${num_proyecto}/${responsable}/${clasificacionPY}/${offset}/${limit}`)
  }
